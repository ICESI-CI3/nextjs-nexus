...
export function setCookie(name: string, value: string, days: number = 7): void {
  ...
  // Use SameSite=None with Secure for cross-origin (ngrok)
  // Use SameSite=Lax for same-origin (localhost)
...
export function removeCookie(name: string): void {
  ...
  // Match the SameSite/Secure settings with setCookie
  const isSecure = window.location.protocol === 'https:';
  const sameSite = isSecure ? 'None' : 'Lax';
  const secureFlag = isSecure ? ';Secure' : '';

  document.cookie = `${name}=;expires=Thu, 01 Jan 1970 00:00:00 GMT;path=/;SameSite=${sameSite}${secureFlag}`;
}
